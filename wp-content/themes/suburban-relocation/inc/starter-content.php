...
function srs_starter_admin_notice() {
	...
	<div class="notice notice-info is-dismissible">
		<p><strong><?php esc_html_e( 'Suburban Relocation theme is ready.', 'suburban-relocation' ); ?></strong> <?php esc_html_e( 'Import the services, locations and menus, then edit the homepage under Appearance → Home Sections.', 'suburban-relocation' ); ?> <a href="<?php echo esc_url( $url ); ?>"><?php esc_html_e( 'Open Starter Content', 'suburban-relocation' ); ?></a></p>
	</div>
	<?php
}
...
function srs_render_starter_content_page() {
	...
	<div class="wrap">
		...
		<p><?php esc_html_e( 'This creates the Home page, five detailed service articles, six primary locations, all archived city service-area pages, supporting pages and navigation menus.', 'suburban-relocation' ); ?></p>
		<ul style="list-style:disc;padding-left:22px">
			<li><?php esc_html_e( 'Every homepage section is edited safely under Appearance → Home Sections without changing layout markup.', 'suburban-relocation' ); ?></li>
...
function srs_import_starter_content() {
	...
	foreach ( srs_starter_services() as $index => $service ) {
		$existing = srs_find_post_by_slug( $service['slug'], 'srs_service' );
		if ( $existing ) {
			$post_id = $existing->ID;
		} else {
	...
	foreach ( srs_starter_primary_locations() as $index => $location ) {
		$existing = srs_find_post_by_slug( $location['slug'], 'srs_location' );
		if ( $existing ) {
			$post_id = $existing->ID;
		} else {
			$post_id = wp_insert_post(
	...
	$home_id = srs_import_page( 'Home', 'home', '', $count );
...
function srs_import_page( $title, $slug, $content, &$count ) {
	$existing = srs_find_post_by_slug( $slug, 'page' );
	if ( $existing ) {
		return $existing->ID;
	}
	$post_id = wp_insert_post(
...
/** Find a slug only inside the requested post type (attachments must not match). */
function srs_find_post_by_slug( $slug, $post_type ) {
	$posts = get_posts(
		array(
			'name'             => $slug,
			'post_type'        => $post_type,
...
function srs_create_starter_menus( $home_id, $service_ids, $location_ids, $page_ids ) {
	...
	$services_parent  = 0;
	$locations_parent = 0;
	$present_services = array();
	$present_locations = array();
	foreach ( $items as $item ) {
		if ( 'custom' === $item->type && 'Services' === $item->title ) {
			$services_parent = $item->ID;
		}
		if ( 'custom' === $item->type && 'Our Locations' === $item->title ) {
			$locations_parent = $item->ID;
		}
		if ( 'srs_service' === $item->object ) {
			if ( 'srs_service' !== get_post_type( $item->object_id ) ) {
				wp_delete_post( $item->ID, true );
			} else {
				$present_services[] = (int) $item->object_id;
			}
		}
		if ( 'srs_location' === $item->object && 'srs_location' === get_post_type( $item->object_id ) ) {
			$present_locations[] = (int) $item->object_id;
		}
	}
	...
	foreach ( $service_ids as $post_id ) {
		if ( ! in_array( (int) $post_id, $present_services, true ) ) {
	...
	foreach ( array_slice( $location_ids, 0, 6 ) as $post_id ) {
		if ( ! in_array( (int) $post_id, $present_locations, true ) ) {
