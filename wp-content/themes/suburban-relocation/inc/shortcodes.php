		...
		<?php while ( $query->have_posts() ) : $query->the_post(); ?>
			<article class="srs-service-card">
				<div class="srs-card-top">
					<span class="srs-card-icon"><?php $icons = array( 'home', 'truck', 'building', 'warehouse' ); echo srs_icon( $icons[ ( $number - 1 ) % 4 ] ); ?></span>
				...
				<a class="srs-text-link" href="<?php the_permalink(); ?>"><?php esc_html_e( 'Explore service', 'suburban-relocation' ); ?> <?php echo srs_icon( 'arrow-up-right' ); ?></a>
			</article>
			<?php ++$number; ?>
		<?php endwhile; ?>
	</div>
		...
		<?php else : ?>
			...
			<form class="srs-quote-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-ajax-url="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post">
				...
				<div class="srs-form-grid">
					...
					<label><span><?php esc_html_e( 'Move date', 'suburban-relocation' ); ?> <small><?php esc_html_e( '(optional)', 'suburban-relocation' ); ?></small></span><input type="date" name="move_date" autocomplete="off"></label>
